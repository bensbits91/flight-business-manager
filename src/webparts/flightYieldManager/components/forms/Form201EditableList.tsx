...
export interface F201EditableListProps {
...
export interface F201EditableListState {
    logs: IListItem[];
    // colGroups: any;
}

class F201EditableList extends React.Component<F201EditableListProps, F201EditableListState> {
    ...
    constructor(props: F201EditableListProps) {
        super(props);
        ...
        this._columns = [
            ...
            { key: 'DepartureDateTime', name: 'Date', fieldName: 'DepartureDateTime', minWidth: 75, maxWidth: 75, isResizable: false },
    ...
    public componentDidMount() {
        // console.log('%c : F201EditableList -> componentDidMount -> this.props', mcc, this.props);
        this._allItems = [];
        if (this.props.logs) {
            this.props.logs.map(log => {
                // console.log('%c : F201EditableList -> componentDidMount -> log', mcc, log);
    ...
    // public componentDidUpdate(prevProps: F201EditableListProps, prevState: F201EditableListState) {
    //     console.log('%c : F201EditableList -> componentDidUpdate -> this.state', mcc, this.state);
    // }
    ...
    public handler_fields(data) {
        console.log('%c : F201EditableList -> handler_fields -> data', mcc, data);
    ...
    public _onRenderItemColumn(item: IListItem, index: number, column: IColumn) {
        ...
        switch (fieldName) {
            ...
            case 'DepartureDateTime':
                return <AsdfDateTimePicker
                    field={{
                        Title: column.name,
                        InternalName: fieldName,
                        TypeAsString: 'DateTIme',
                        Description: '',
                        value: item.DepartureDateTime
                    }}
                    showDate
                    showTime={false}
                    label={false}
    ...
    public _onColumnResize(column?: IColumn, newWidth?: number, columnIndex?: number) {
        // console.log('%c : F201EditableList -> _onColumnResize -> column', mcc, column);
        // console.log('%c : F201EditableList -> _onColumnResize -> newWidth', mcc, newWidth);
        // console.log('%c : F201EditableList -> _onColumnResize -> columnIndex', mcc, columnIndex);
        ...
        // console.log('%c : F201EditableList -> _onColumnResize -> changedGroup', mcc, changedGroup);
        // changedGroup.colWidth = newWidth;
        // console.log('%c : F201EditableList -> _onColumnResize -> changedGroup', mcc, changedGroup);

        // console.log('%c : F201EditableList -> _onColumnResize -> newGroups', mcc, newGroups);
    ...
    //     console.log('%c : F201EditableList -> ev', mcc, ev);
    //     console.log('%c : F201EditableList -> text', mcc, text);
    //     this.setState({
    ...
    public render() {
        ...
        const el_listHeading = <div className='listHeading'>{aircraftName}</div>;
        ...
        const el_list = logs ? <Fabric className='listFabric'>
...
export default F201EditableList;